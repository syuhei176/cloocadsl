function ToolExplorer(toolController, wb) {
 ...
 * レンダリング
 */
ToolExplorer.prototype.refresh = function() {
	var self = this;
	/*
	 * メタデータコントローラからデータ取得
	 */
	var notation_members = this.toolController.getlist();
	/*
	 * パッケージを読み込みツリー状に表示する
	 */
...
/*	    title: 'Package Explorer',
	...
	Ext.getCmp('tool-explorer').removeAll();
	Ext.getCmp('tool-explorer').add(this.panel);
}

ToolExplorer.prototype.create = function() {
	var self = this;
...
ToolExplorer.prototype.open = function() {
	/*
...
ToolExplorer.prototype.resetting_option = function() {
	/*
	 * 選択されているパッケージの設定を変更するダイアログを表示する
	*/
}

ToolExplorer.prototype.del = function() {
	var self = this;
	/*
	 * 選択されているパッケージを削除する
	*/
 ...
 * private function
 */
ToolExplorer.prototype.init_contextmenu = function() {
	var self = this;
	/*
	 * 右クリックメニューの設定
	 */
	...
	this.panel.on('itemmousedown',function(view, record, item, index, event) {
		self.selectedTool = record.data.uri;
		if(event.button == 2) {
			if(record.data.root) {
